#!/usr/bin/env python
"""Quick test script to run diarization on AMI files without database."""
...
async def main():
    """Run diarization on 4 AMI test files."""
    
    # Audio files
    audio_dir = Path("data/ami/audio/test")
    annotation_dir = Path("data/ami/annotations/test")
    
    audio_files = [
        "EN2002a.Mix-Headset.wav",
        "EN2002b.Mix-Headset.wav",
        "EN2002c.Mix-Headset.wav",
        "EN2002d.Mix-Headset.wav",
    ]
    ...
    logger.info(f"Starting benchmark on {len(audio_files)} AMI files...")
    logger.info("Mock diarization mode")    
    logger.info("=" * 80)
